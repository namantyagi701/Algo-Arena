...
function ProblemDescription({ problem, currentProblemId, onProblemChange, allProblems }) {
  return (
    <div className="h-full overflow-y-auto bg-base-200">
      {/* HEADER SECTION */}
      <div className="p-6 bg-base-100 border-b border-base-300/50">
            ...
            {problem.examples.map((example, idx) => (
              <div key={idx}>
                ...
                <div className="bg-base-200/80 rounded-lg p-4 font-mono text-sm space-y-1.5 border border-base-content/5">
                  ...
                  {example.explanation && (
                    <div className="pt-2 border-t border-base-300/50 mt-2">
          ...
          <ul className="space-y-2 text-base-content/90">
            {(problem.constraints || []).map((constraint, idx) => (
              <li key={idx} className="flex gap-2">
                ...
                <code className="text-sm bg-base-200/50 px-1.5 py-0.5 rounded">{constraint}</code>
              </li>
            ))}
          </ul>
        </div>
